...
export function renderCatalog(snapshot) {
  const catalog = Object.fromEntries(
    snapshot.tools.map(name => {
      const { title, description, pageDescriptions, outputSchema } = snapshot.surface[name];
      return [name, { title, description, ...(pageDescriptions ? { pageDescriptions } : {}), outputSchema }];
    }),
  );
  return `/**
