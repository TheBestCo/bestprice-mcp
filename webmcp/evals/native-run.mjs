 ...
 *
 * A real run appends to runs.v11.json (runs.v<N>.json with --dataset=v<N>) and writes artifacts/, so
...
/* Dataset 11.0.0 is current: it grades contract 1.9 as revised on 2026-09-25 (see dataset-v11.js).
 * `--dataset=v2` … `--dataset=v10` still run a frozen set into its own ledger. A run's dataset version is read from the file, never
 * assumed. */
...
const main = async () => {
  ...
  const selectedDataset = DATASETS[options.dataset ?? 'v11'];
