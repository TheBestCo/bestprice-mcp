 ...
 *
 * A real run appends to runs.v16.json (runs.v<N>.json with --dataset=v<N>) and writes artifacts/, so
...
/* Dataset 16.0.0 is current: it grades contract 2.6 (see dataset-v16.js). `--dataset=v2` …
 * `--dataset=v15` still run a frozen set into its own ledger. A run's dataset version is read from the file, never
 * assumed. */
const DATASETS = Object.freeze({
  ...
  /* 5.0.0 with listing-011 and multi-006 corrected; see dataset-v6.js. */
  v6: { cases: join(EVAL_DIR, 'natural-language-cases.v6.json'), ledger: join(EVAL_DIR, 'runs.v6.json') },
  /* 6.0.0 with home-003, multi-006, multi-002 and multi-008 corrected; see dataset-v7.js. */
  v7: { cases: join(EVAL_DIR, 'natural-language-cases.v7.json'), ledger: join(EVAL_DIR, 'runs.v7.json') },
  /* 7.0.0 with listing-010 and neg-007 corrected; see dataset-v8.js. */
  v8: { cases: join(EVAL_DIR, 'natural-language-cases.v8.json'), ledger: join(EVAL_DIR, 'runs.v8.json') },
  /* 8.0.0 graded against contract 1.8: its argument rules, reads and structured search result; see
   * dataset-v9.js. The versions before it stay frozen with their runs as 1.6/1.7 history. */
  v9: { cases: join(EVAL_DIR, 'natural-language-cases.v9.json'), ledger: join(EVAL_DIR, 'runs.v9.json') },
  /* 9.0.0 graded against contract 1.9: get_product_details admitted as an extra read, its `include`
   * list graded; see dataset-v10.js. 9.0.0 stays frozen as contract 1.8 history. */
  v10: { cases: join(EVAL_DIR, 'natural-language-cases.v10.json'), ledger: join(EVAL_DIR, 'runs.v10.json') },
  /* 10.0.0 graded against 1.9's 2026-09-25.7 revision: search constraints, digits-only product ids,
   * get_product_details.navigate; see dataset-v11.js. 10.0.0 stays frozen. */
  v11: { cases: join(EVAL_DIR, 'natural-language-cases.v11.json'), ledger: join(EVAL_DIR, 'runs.v11.json') },
  /* 11.0.0 graded against 1.9's 2026-09-25.12 revision: offer offset, single-filter removal,
   * show_chart, show_offer without merchant_id, load_more/show_chart false as extra reads; see
   * dataset-v12.js. 11.0.0 stays frozen. */
  v12: { cases: join(EVAL_DIR, 'natural-language-cases.v12.json'), ledger: join(EVAL_DIR, 'runs.v12.json') },
  /* 12.0.0 graded against contract 2.0: the removed tools replaced by their successors (open_product,
  ...
  /* 14.0.0 graded against contract 2.2: open_search_results opens the results_url a search returned, so
   * multi-001 and multi-002 search first; see dataset-v15.js. 14.0.0 stays frozen. */
  v15: { cases: join(EVAL_DIR, 'natural-language-cases.v15.json'), ledger: join(EVAL_DIR, 'runs.v15.json') },
  /* 15.0.0 graded against contract 2.6 (2.3 to 2.6): the list and comparison reads admitted as extra
   * reads, the Shopping Brain's structured inputs; see dataset-v16.js. 15.0.0 stays frozen. */
  v16: { cases: join(EVAL_DIR, 'natural-language-cases.v16.json'), ledger: join(EVAL_DIR, 'runs.v16.json') },
});
...
const main = async () => {
  ...
  const selectedDataset = DATASETS[options.dataset ?? 'v16'];
