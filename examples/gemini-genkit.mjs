...
export const genkitMcpConfig = {
  ...
  allowedTools: ['get_shopping_decision', 'search_products', 'compare_offers', 'get_price_history'],
};
...
export function validateGenkitConfig(config = genkitMcpConfig) {
  ...
  if (!Array.isArray(config.allowedTools) || config.allowedTools.length !== 4) {
    throw new Error('Allowed tools must contain exactly the 4 published read-only tools');
  }
  return true;
}
