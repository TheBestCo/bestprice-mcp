...
1. `Find Sony WH-1000XM5 headphones under €300 on BestPrice.`
2. Using a returned `product_id`, ask: `Compare delivered offers to postcode 11527.`
3. Using the same `product_id`, ask: `Is today's price low compared with the last 180 days?`

Pass only if Perplexity discovers exactly `search_products`, `compare_offers`,
and `get_price_history`; each call succeeds; product links open a canonical
BestPrice product path; item price and shipping stay separate; and unknown
shipping is not described as free.
