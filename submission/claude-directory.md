...
> Make a shopping decision, search products, compare delivered prices, and
> check price history in Greece.

Suggested description:

> BestPrice Shopping gives Claude read-only access to the shared BestPrice
> Shopping Brain, product search, current offer comparison, and price history
> for safe physical products across the
...
- `get_shopping_decision`: returns one bounded, evidence-backed recommendation,
...
All four tools publish a human-readable title plus `readOnlyHint: true`,
...
2. `Βρες μου Sony WH-1000XM5 έως 300 ευρώ.`
3. `Σύγκρινε τις προσφορές για το προϊόν που επέλεξα, με παράδοση στο 10558.`
4. `Είναι η σημερινή τιμή χαμηλή σε σχέση με τις τελευταίες 180 ημέρες;`
...
Three historical response-only captures for the data tools are retained:
...
Capture one fresh `get_shopping_decision` response and refresh these data-tool
screenshots after the exact-version production canary. The retained images are
interface references, not evidence for the new four-tool release or static
product claims.
